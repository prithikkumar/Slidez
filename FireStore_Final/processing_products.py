...
INPUT_FILE = r'C:\Users\prith\OneDrive\Desktop\Slidez\FireStorePush\FireStore_Final\Raw_Data\all_successful_products.jsonl'  # Your input
OUTPUT_FILE = r'C:\Users\prith\OneDrive\Desktop\Slidez\FireStorePush\Output_FireStore\products_final_28_06.jsonl'
...
def fix_images_array(images: Optional[List[Dict[str, Any]]], featured_image: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Step 1: Deduplicate based on (position, mediaURL)
    for img in images:
        key = (img.get("position"), img.get("mediaURL"))
        if key not in seen_keys:
            seen_keys.add(key)
            deduped_images.append(img)

    # Step 2: Add featuredImage only if it's valid and not already in images
    if featured_image and featured_image.get("mediaURL"):
        featured_key = (0, featured_image["mediaURL"])
        if featured_key not in seen_keys:
            featured_entry = {
            ...
            deduped_images.insert(0, featured_entry)
